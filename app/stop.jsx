...
const roundTo = nearest => n => Math.round(nearest * n) / nearest;
const roundTen = roundTo(10);

const distanceToStart = ({stop, location}) => roundTen(
  latLon(location).distanceTo(
    latLon({coords: stop})
  ) / KILOMETERS_PER_MILE
);
...
<header className='stop-header' style={{backgroundImage: gradient(getStreetViewUrl(stop))}}>
